	...
	window.apply = $.fn.apply = {
		...
		getNotesInfoByApplyNo:function(){
			$.ajax({
				...
				success : function(data){
					//console.log(data);
					$("#pjtab").attr({"href":"#pjxx", "data-toggle":"tab"});
					if(data.errorCode == "0"){
						if(data.dataBody.applyTicket){
							...
							$("#loanDescpj").val(applyList.remark);
...
$(document).ready(function($){
	...
	//票据信息下一步
	$("#pjxxSubmit").click(function(){
		if($("#pjxxform").valid() == true){
			var data = {
				applyNo: applyNo,
				applyId: applyId,
	...
	$("#upFront").change(function(){
		apply.getFile($(this).val(), 'upFront', "front","F038", "pjxxform","imgFront");
	})	
	//上传票据背面
	$("#back").click(function(){
		$("#upBack").click();
	...
	$("#upBack").change(function(){
		apply.getFile($(this).val(), 'upBack', "back","F039", "pjxxform","imgBack");
	})
	//上传合同附件
	...
	$("#htfj").change(function(){
		apply.getFile($(this).val(), 'htfj', "htfjCont","F040", "yszform");
	})
	//上传发票附件
	...
	$("#fpfj").change(function(){
		apply.getFile($(this).val(), 'fpfj', "fpfjCont","F041", "yszform");
	})
	//上传其他附件
	...
	$("#qtfj").change(function(){
		apply.getFile($(this).val(), 'qtfj', "qtfjCont","F042", "yszform");
	})
