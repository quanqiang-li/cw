(function($) {
	...
	window.apply = $.fn.apply = {
		...
		/* 应收账款人认证 */
		personalVerify:function(datalist){
			$.jBox.tip("正在提交，请稍后...", 'loading');
			$.ajax({
				url: "/unifiedInterfaceAccessPlatform/verifyNameAndMobileNum",
				type: "get",
				data: {
					realName : datalist.buyerContactName,
					mobileNum: datalist.buyerContactMobile
				},
				dataType: "json",
				cache:false,
				success : function(data){
...
$(document).ready(function($){
	...
	$("#htfj").change(function(){
		apply.getFile($(this).val(), 'htfj', "htfjCont","F004");
	})
	//上传发票附件
	...
	$("#fpfj").change(function(){
		apply.getFile($(this).val(), 'fpfj', "fpfjCont","F005");
	})
	//上传其他附件
	...
	$("#qtfj").change(function(){
		apply.getFile($(this).val(), 'qtfj', "qtfjCont","F006");
	})
